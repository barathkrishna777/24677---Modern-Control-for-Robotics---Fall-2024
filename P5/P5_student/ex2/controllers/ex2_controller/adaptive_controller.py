...
class AdaptiveController(BaseController):
    ...
    def initializeGainMatrix(self):
        ...
        A = np.vstack((np.hstack((Ap, zero_np_m)), np.hstack((Cp, zero_m_m))))
        B = np.vstack((Bp, zero_m_m))
        Bc = np.vstack((zero_np_m, -I_m_m))
        C = np.hstack((Cp, zero_m_m))
        D = np.zeros((m, m))
        
        sys_ct = signal.StateSpace(A, B, C, D)
        sys_dt = sys_ct.to_discrete(self.delT)

        sysc_ct = signal.StateSpace(A, Bc, C, D)
        sysc_dt = sysc_ct.to_discrete(self.delT)
